"""
Pure-Python implementation of the Highway overlapping community detection
algorithm for CDlib.
...
The function signature mirrors the C++ backend wrapper, but this file has no
C++ binary, subprocess, temporary file, or backend dependency.
"""
...
from typing import Dict, Hashable, List, MutableMapping, Sequence, Tuple
...
class LocalRefineConfig:
    ...
    confidence_self_fraction_weight: float = 0.85
    confidence_low_entropy_weight: float = 0.15
    pattern_confidence_floor: float = 0.05
    pattern_confidence_ceiling: float = 1.00
    update_strength: float = 0.50
    node_mode_power: float = 1.50
    pattern_target_mix: float = 0.75
    target_sharpen_gamma: float = 1.20
    min_abs_mass_to_keep: float = 1e-8
...
def _topk_from_dense(values: Sequence[float], k: int) -> MembershipRow:
    """Mimic the C++ topk_indices_values behavior for dense anchor scores."""
    k = max(0, min(int(k), len(values)))
    if k <= 0:
        return []
    items = list(enumerate(values))
    items.sort(key=lambda x: (-x[1], x[0]))
    return [(cid, float(value)) for cid, value in items[:k] if value > 0.0]
...
def _stable_softmax(items: MembershipRow, tau: float, eps: float = 1e-12) -> MembershipRow:
    ...
    max_value = max(v / tau for _, v in items)

    exps = []
    total = 0.0
    for cid, value in items:
        ev = exp((value / tau) - max_value)
        exps.append((cid, ev))
        total += ev
...
def _relabel_graph_to_contiguous(
    ...
    The returned adjacency lists represent an undirected graph and are sorted
...
def _build_highway_adjacency(
    ...
    """
    Build the sparse Highway backbone using the mod-jaccard hybrid score.
...
def _propagate_anchor_memberships(
    ...
    for _ in range(T):
        ...
        for v in range(n):
            ...
            if not any(value > 0.0 for value in acc):
                next_state[v] = list(state[v])
                continue

            top = _topk_from_dense(acc, min(r, k))
        ...
        if damping < 1.0:
            ...
            for v in range(n):
                ...
                top = _topk_from_dense(mixed, min(r, k))
                mixed_state[v] = _stable_softmax(top, cfg.tau, cfg.eps)
...
def _normalized_entropy_counts(counts: MutableMapping[int, int]) -> float:
    total = 0.0
    k = 0
...
def _neighbor_anchor_consensus(
    ...
    if v < 0 or v >= len(adj) or v >= len(state):
        return {}

    for u in adj[v]:
        if u < 0 or u >= len(state):
            continue
        for cid, prob in state[u]:
...
def _refine_anchor_preserving_patterns(
    ...
    """
    Anchor-preserving pattern refinement.
    ...
    # Estimate pattern structural confidence on the full graph.
    # Count undirected edges once, matching the C++ implementation.
    for u in range(n):
        pu = node_pid[u]
        if pu < 0:
            continue
    ...
    # Build anchor-space pattern targets by averaging propagated memberships.
    for v, row in enumerate(state):
        pid = node_pid[v]
        if pid < 0:
            continue
    ...
    for pid, pat in enumerate(patterns):
        ...
        incident_units = 2 * internal_edges + external_edges

        if incident_units > 0:
            self_fraction = float(2 * internal_edges) / float(incident_units)
        else:
            self_fraction = 0.0

        neighbor_entropy = _normalized_entropy_counts(pat["ext_counts"])
    ...
    local_adj = backbone_adj if len(backbone_adj) == n else full_adj
    mix = _clamp(cfg.pattern_target_mix, 0.0, 1.0)
    update_strength = _clamp(cfg.update_strength, 0.0, 1.0)
    mode_power = max(0.0, cfg.node_mode_power)
    ...
    for v in range(n):
        ...
        q: Dict[int, float] = defaultdict(float)
        for cid, prob in target.items():
            if cid >= 0 and prob > 0.0:
                q[cid] += mix * prob

        q_neighbor = _neighbor_anchor_consensus(state, local_adj, v)
        for cid, prob in q_neighbor.items():
            if cid >= 0 and prob > 0.0:
                q[cid] += (1.0 - mix) * prob
        ...
        same_ratio = _same_pattern_neighbor_ratio(local_adj, node_pid, v)
        node_factor = _clamp(same_ratio, 0.0, 1.0) ** mode_power
        lambda_v = _clamp(update_strength * _clamp(pattern_confidence[pid], 0.0, 1.0) * node_factor, 0.0, 1.0)
        ...
        for cid, prob in state[v]:
            if cid >= 0 and prob > 0.0:
                blended[cid] += (1.0 - lambda_v) * prob

        for cid, prob in q.items():
            if cid >= 0 and prob > 0.0:
                blended[cid] += lambda_v * prob

        blended = _normalize(blended)
        blended = _sharpen(blended, cfg.target_sharpen_gamma)

        items = [
            (cid, prob)
            for cid, prob in blended.items()
...
def _decode_state_labels(
    ...
    """
    Decode each node's top-r anchor distribution into anchor labels.

    This follows the C++ main.cpp decode_state_labels behavior: keep labels with
    mass >= theta up to max_memberships; if none pass the threshold, keep the
...
def _build_communities_from_memberships(
    ...
    for vid, labels in enumerate(node_memberships):
        if vid < 0 or vid >= len(nodes):
            continue
        for lab in labels:
            if lab >= 0:
                comm_to_nodes[lab].append(nodes[vid])

    communities = []
    for lab in sorted(comm_to_nodes):
        comm = comm_to_nodes[lab]
...
def highway_nx(
    ...
    This function exposes the same Python-facing API as the C++ backend wrapper,
    but executes the full algorithm in pure Python.
    ...
    if sum(len(neigh) for neigh in full_adj) == 0:
    ...
    rcfg = LocalRefineConfig(
        ...
        confidence_self_fraction_weight=local_confidence_self_fraction_weight,
        confidence_low_entropy_weight=local_confidence_low_entropy_weight,
        pattern_confidence_floor=local_pattern_confidence_floor,
        pattern_confidence_ceiling=local_pattern_confidence_ceiling,
        update_strength=local_update_strength,
        node_mode_power=local_node_mode_power,
        pattern_target_mix=local_pattern_target_mix,
        target_sharpen_gamma=local_target_sharpen_gamma,
        min_abs_mass_to_keep=local_min_abs_mass_to_keep,
        renormalize=local_renormalize,
    )
    ...
    communities = _build_communities_from_memberships(
